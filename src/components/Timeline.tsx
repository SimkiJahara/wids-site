    ...
    <section className="py-20 bg-wids-dark relative">
      <div className="container mx-auto px-4">
        <FadeIn className="text-center mb-16">
          <h2 className="text-3xl font-bold mb-4 text-white">{timeline.title}</h2>
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <div className="space-y-12">
            {timeline.events.map((event, index) => {
              ...
              return (
                <SlideUp key={index} delay={index * 0.1} className={`flex flex-col md:flex-row items-center ${isEven ? 'md:flex-row-reverse' : ''}`}>
                  <div className="flex-1 w-full md:w-1/2 p-4">
                    <div className={`p-6 rounded-xl border ${isPast ? 'bg-white/5 border-white/5 opacity-50' : 'glass-card'} transition-all hover:border-wids-teal/30`}>
                      <div className="flex items-center gap-3 mb-2">
                        <Calendar className={`w-5 h-5 ${isPast ? 'text-slate-500' : 'text-wids-teal'}`} />
                        <span className={`font-bold ${isPast ? 'text-slate-500' : 'text-white'}`}>{event.date}</span>
                      </div>
                      <h3 className="text-xl font-bold mb-1 text-white">{event.title}</h3>
                      <span className={`text-xs font-semibold px-2 py-1 rounded-full ${isPast ? 'bg-white/5 text-slate-500' : 'bg-wids-gold/20 text-wids-gold'}`}>
